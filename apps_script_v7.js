// COPIHUE HERRAMIENTAS + CANALES TV - Apps Script Web App v7
// v7: soporte columna 'bumpeo' además de 'ver' — ambos nombres funcionan
...
function doGet(e) {
  ...
  return ContentService
    .createTextOutput(JSON.stringify({ status: 'ok', msg: 'API v7 activa' }))
...
function getHerramientas() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    herramientas.push({
      ...
      url:           col.url >= 0 && col.url !== -1 ? (row[col.url] || '').toString().trim() : '',
...
function updateRow(data) {
  ...
  // Mapa de campos → columna en la sheet
  // 'ver' y 'version' apuntan a la columna VER
  // Resuelve la columna de versión: acepta VER o bumpeo
  ...
  if (data.campo.toLowerCase() === 'ver' || data.campo.toLowerCase() === 'version') {
...
function respHtml(titulo, msg, color) {
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
    ...
    'h2{color:' + color + ';margin-bottom:.5rem;font-size:24px;}' +
    'p{color:#7a8099;font-size:14px;margin-bottom:1.5rem;}' +
    '.closing{color:#3e4460;font-size:12px;}</style>' +
    '<script>setTimeout(function(){document.querySelector(".closing").textContent="Podés cerrar esta pestaña.";},1000);</script></head>' +
    '<body><div class="box"><h2>' + titulo + '</h2><p>' + msg + '</p>' +
    '<div class="closing">Cerrando…</div></div></body></html>';
...
function agregarFila(data) {
  ...
  set('NOMBRE', t.nombre);
  // Intenta escribir en VER primero, si no existe usa BUMPEO
