// COPIHUE HERRAMIENTAS + CANALES TV - Apps Script Web App v10
...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.action === 'enviarNotaDirecta') {
    return enviarNota({
      app:     e.parameter.app     || '',
  ...
  return ContentService
    .createTextOutput(JSON.stringify({ status: 'ok', msg: 'API v10 activa' }))
...
function enviarNota(data) {
  ...
  // Enviar mail
  const asunto = '[Herramientas] ' + (app || 'Nota') + (ver ? ' v' + ver : '') + ' - Nota rapida';
  const nl = '\n';
  const cuerpo = 'Nota rapida registrada desde el panel de Herramientas.' + nl + nl +
    'App: ' + (app || '-') + (ver ? ' (v' + ver + ')' : '') + nl +
    'Fecha: ' + fecha + nl + nl +
    'Nota:' + nl + nota + nl + nl +
    '---' + nl + 'Guardado en hoja NOTAS_RAPIDAS de tu planilla.';
